...
def get_pasal_by_params(query_params: Dict[str, str]) -> List[Dict[str, str]]:
    ...
    try:
        output = []
        total_elements = PASAL_DB.count_documents({})
        ...
        for pasal in PASAL_DB.find(query).sort(sort["field"], sort["direction"]).skip(pagination["limit"] * pagination["page"]).limit(pagination["limit"]):
